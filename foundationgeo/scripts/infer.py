...
@click.option('--fov_x', 'fov_x_', type=float, default=None, help='If camera parameters are known, set the horizontal field of view in degrees. Otherwise, foundationgeo will estimate it.')
@click.option('--output', '-o', 'output_path', default='./output', type=click.Path(), help='Output folder path')
@click.option('--pretrained', 'pretrained_model_name_or_path', type=str, required=True, help='Pretrained model checkpoint path or Hugging Face repo id.')
@click.option('--version', 'model_version', type=click.Choice(['base', 'v1']), default='v1', help='Model version. Use "base" for the first-stage model and "v1" for FoundationGeo.')
...
def main(
    ...
    from foundationgeo.utils.io import save_glb, save_ply
    from foundationgeo.utils.vis import colorize_depth, colorize_normal, colorize_scalar_field, colorize_vector_field
    from foundationgeo.utils.geometry_numpy import depth_occlusion_edge_numpy
    import utils3d
    ...
    if model_version == 'base':
        from foundationgeo.model.base import FoundationGeo
    elif model_version == 'v1':
        from foundationgeo.model.v1 import FoundationGeo
    else:
        raise ValueError(f'Unsupported model version: {model_version}')
    model = FoundationGeo.from_pretrained(pretrained_model_name_or_path).to(device).eval()
    if use_fp16:
        model.half()
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Inference', disable=len(image_paths) <= 1)):
        ...
        depth_metric = output['depth_metric'].cpu().numpy() if 'depth_metric' in output else None
        points_metric = output['points_metric'].cpu().numpy() if 'points_metric' in output else None
        scalefield = output['scalefield'].cpu().numpy() if 'scalefield' in output else None
        delta = output['delta'].cpu().numpy() if 'delta' in output else None
        points_export = points_metric if points_metric is not None else points
        depth_export = depth_metric if depth_metric is not None else depth
        ...
        if save_maps_:
            ...
            if depth_metric is not None:
                cv2.imwrite(str(save_path / 'depth_metric_vis.png'), cv2.cvtColor(colorize_depth(depth_metric), cv2.COLOR_RGB2BGR))
                cv2.imwrite(str(save_path / 'depth_metric.exr'), depth_metric, [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT])
            if points_metric is not None:
                cv2.imwrite(str(save_path / 'points_metric.exr'), cv2.cvtColor(points_metric, cv2.COLOR_RGB2BGR), [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT])
            if scalefield is not None:
                # cv2.imwrite(str(save_path / 'scalefield.exr'), scalefield, [cv2.IMWRITE_EXR_TYPE, cv2.IMWRITE_EXR_TYPE_FLOAT])
                cv2.imwrite(str(save_path / 'scalefield.png'), cv2.cvtColor(colorize_scalar_field(scalefield[..., 0], mask), cv2.COLOR_RGB2BGR))
            if delta is not None:
                delta_magnitude = np.linalg.norm(delta, axis=-1)
                cv2.imwrite(str(save_path / 'delta_mag.png'), cv2.cvtColor(colorize_depth(delta_magnitude, mask), cv2.COLOR_RGB2BGR))
                cv2.imwrite(str(save_path / 'delta_vis.png'), cv2.cvtColor(colorize_vector_field(delta, mask), cv2.COLOR_RGB2BGR))
        ...
        if save_glb_ or save_ply_ or show:
            mask_cleaned = mask & ~utils3d.numpy.depth_edge(depth_export, rtol=threshold)
            if normal is None:
