...
from foundationgeo.train.dataloader import TrainDataLoaderPipeline
from foundationgeo.train.losses import (
...
from foundationgeo.train.utils import build_optimizer, build_lr_scheduler
...
from foundationgeo.utils.tools import key_average, recursive_replace, CallbackOnException, flatten_nested_dict
...
def main(
    ...
    with accelerator.local_main_process_first():
        from foundationgeo.model import import_model_class_by_version
        FoundationGeo = import_model_class_by_version(config['model_version'])      
        model = FoundationGeo(**config['model'])
    ...
    if torch.version.hip and isinstance(model, torch.nn.parallel.DistributedDataParallel):
        ...
        from foundationgeo.model.utils import sync_ddp_hook
    ...
    with (
        ...
        for i_step in range(initial_step, num_iterations):
            ...
            while i_accumulate < gradient_accumulation_steps:
                ...
                image, gt_depth, gt_normal, gt_mask, gt_mask_fin, gt_mask_inf, gt_intrinsics, label_type, is_metric = batch['image'], batch['depth'], batch['normal'], batch['depth_mask'], batch['depth_mask_fin'], batch['depth_mask_inf'], batch['intrinsics'], batch['label_type'], batch['is_metric']
                image, gt_depth, gt_normal, gt_mask, gt_mask_fin, gt_mask_inf, gt_intrinsics = image.to(device), gt_depth.to(device), gt_normal.to(device), gt_mask.to(device), gt_mask_fin.to(device), gt_mask_inf.to(device), gt_intrinsics.to(device)
                ...
                with accelerator.accumulate(model):
                    ...
                    for i in range(current_batch_size):
                        ...
                        for k, v in config['loss'][label_type[i]].items():
                            ...
                            elif v['function'] in {'normal_map', 'normal_map_loss'}:
                                pred_normal, _ = utils3d.torch.points_to_normals(pred_points[i], gt_mask[i])
                                loss_dict[k], misc_dict[k] = normal_map_loss(pred_normal, gt_normal[i])
