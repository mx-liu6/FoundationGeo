...
from foundationgeo.utils.vis import colorize_depth, colorize_normal
...
def main(
    ...
    with accelerator.local_main_process_first():
        from foundationgeo.model.base import FoundationGeo as BaseFoundationGeo
        model_config = dict(config['model'])
        freeze_encoder = bool(model_config.pop('freeze_encoder', freeze_encoder))
        model = BaseFoundationGeo(**model_config)
        if freeze_encoder:
    ...
    if enable_gradient_checkpointing and not freeze_encoder:
                    ...
                    for i in range(current_batch_size):
                        ...
                        for k, v in config['loss'][label_type[i]].items():
                            ...
                            if v['function'] == 'affine_invariant_global_loss':
                                ...
                                raise ValueError(f'Unsupported base-model loss function: {v["function"]}')
