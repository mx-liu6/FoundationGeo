...
class Baseline(MGEBaselineInterface):
    ...
    def __init__(self, num_tokens: int, resolution_level: int, pretrained_model_name_or_path: str, use_fp16: bool, device: str = 'cuda:0', version: str = 'v1'):
        ...
        elif version == 'v1_wo_rayfield':
            from foundationgeo.model.v1_wo_rayfield import FoundationGeo
        else:
            raise ValueError(f'Unsupported FoundationGeo version: {version}')
        self.version = version

        self.model = FoundationGeo.from_pretrained(pretrained_model_name_or_path).to(device).eval()
    ...
    @click.option('--version', type=click.Choice(['base', 'v1', 'v1_wo_rayfield']), default='v1')
    @staticmethod
    ...
    def infer(self, image: torch.FloatTensor, intrinsics: Optional[torch.FloatTensor] = None):
        ...
        if self.version in {'v1', 'v1_wo_rayfield'}:
            return {
                'points_scale_invariant': output['points'],
                'depth_scale_invariant': output['depth'],
                'intrinsics': output['intrinsics'],
                'points_metric': output['points_metric'],
                'depth_metric': output['depth_metric'],
                'mask': output['mask'],
                'scalefield': output.get('scalefield'),
            }
        else:
            return {
                'points_scale_invariant': output['points'],
